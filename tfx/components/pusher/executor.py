...
class Executor(base_executor.BaseExecutor):
  """TFX Pusher executor to deploy the new TF model on tf.serving.

  The Pusher component is used to deploy a validated model to a serving
  environment using tf.serving.  Pusher depends on the outputs of ModelValidator
  to determine if a model is ready to push. A model is considered to be safe to
  push only if ModelValidator has marked it as BLESSED. A push action delivers
  the model exports produced by Trainer to the destination defined in the
  ...
  For more details on the Pusher itself, please refer to
  ...
  def CheckBlessing(self, input_dict,
    ...
    Args:
      input_dict: Input dict from input key to a list of artifacts:
        - model_blessing: model blessing path from model_validator.  This will
        be BLESSED if ModelValidator determined the new model is superior to
        the production model and NOT_BLESSED if not.
